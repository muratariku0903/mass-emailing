...
## 6. 宛先DELETE→INSERTの安全性改善（重要度: 中）

### 現状のリスク

CSV差し替え時に「旧宛先DELETE → 新CSV INSERT」の順序で処理しているが、
INSERT途中でワークフローが失敗した場合、旧宛先が消えた状態で新宛先が不完全になる。

### 改善案: バッチ番号方式

`notification_recipients` に `batch_id` カラムを追加する。

```sql
ALTER TABLE notification_recipients ADD COLUMN batch_id BIGINT NOT NULL DEFAULT 0;
```

**CSV差し替えフロー**:
1. 新しい `batch_id` を採番（例: UNIX タイムスタンプ or シーケンス）
2. 新CSVの宛先を `batch_id = 新番号` で INSERT
3. `notification_campaigns` の参照 `batch_id` を新番号に更新
4. 旧 `batch_id` のレコードを非同期で DELETE

```mermaid
flowchart TD
    A[新batch_id採番] --> B[新CSV宛先を INSERT<br/>batch_id = 新番号]
    B --> C[campaigns.current_batch_id を更新]
    C --> D[旧batch_idのレコードを非同期DELETE]

    B -->|途中で失敗| E[旧データは残っている<br/>current_batch_idは旧のまま<br/>→ 安全にリトライ可能]
```

**メリット**:
- INSERT失敗時に旧データが保持される
- ロールバックが容易
- 送信中にCSV差し替えが走っても（ステータスで防いでいるが念のため）データ不整合が起きない
...
| **P1（推奨）** | 6. DELETE→INSERT安全性 | データ消失リスクの排除 |
